...
table(phsmethods::chi_check(PDS$chi_number)) #some invalid CHI in PDS
...
SC_first <-SC %>% group_by(grouping_chi) %>%
  arrange(diagnosis_date) %>%
  slice(1) %>% 
  select(chi, UPI_NUMBER, diagnosis_date, chi_date_of_birth, chi_date_of_birth, chi_postcode,
        submitted_postcode, submitted_ethnic_group, ch_postcode, ch_admission_date, date_type) %>%
  mutate(dementia_type = "99 Social care flag")



table(SC$financial_quarter_care_first,SC$quarter_client, useNA = "always")
table(is.na(SC$start_date_of_care))
table(is.na(SC$start_date_of_care), SC$quarter_client, useNA = "always")
##Find best date - most have ab actual date. some who have none do have "quarter client"
table(lubridate::year(SC$start_date_of_care))
##some dates are placeholders (1900) or just wrong (1936??) remove dates before 1994
###dates back to the 90s are plausible for long term care
SC_first <- SC_first %>%
